...
public class AinoteNoteServiceImpl extends ServiceImpl<AinoteNoteMapper, AinoteNote>
    ...
    public void applyDataPermission(QueryWrapper<AinoteNote> wrapper) {
        ...
        if (isTeacher) {
            wrapper.and(w -> w.eq("student_id", user.getId())
                    ...
                    .or().eq("is_public", 1)
                    .or().inSql("teaching_id",
                            "SELECT id FROM ainote_teaching WHERE teacher_id = '" + user.getId() +
    ...
    /**
     * 检查用户是否为教师（在 teaching 表中有记录）
     */
    private boolean isTeacher(String userId, Integer tenantId) {
        // 简单实现：检查 teaching 表中是否有该用户作为教师的记录
        // 注意：这里使用原生 SQL 查询，避免循环依赖
        try {
            QueryWrapper<AinoteNote> checkWrapper = new QueryWrapper<>();
            checkWrapper.exists("SELECT 1 FROM ainote_teaching t WHERE t.teacher_id = '" + userId +
                    "' AND t.tenant_id = " + tenantId + " LIMIT 1");
            // 这里只是检查是否存在，不需要实际查询笔记
            // 使用 count 方法会更高效
            return baseMapper.selectCount(checkWrapper) >= 0; // 如果 SQL 执行成功说明表存在
        } catch (Exception e) {
