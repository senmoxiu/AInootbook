...
public class AinoteNoteController extends JeecgController<AinoteNote, IAinoteNoteService> {
    ...
    @GetMapping(value = "/semanticSearch")
    @RequiresPermissions("ainote:note:list")
    public Result<List<SemanticSearchResultDTO>> semanticSearchByScope(
        ...
        NoteSearchScope searchScope;
        try {
            searchScope = NoteSearchScope.valueOf(scope.trim().toUpperCase(Locale.ROOT));
        } catch (Exception e) {
            return Result.error("scope 参数非法，支持 personal/public/course/all");
        }

        Set<String> roles = parseRoles(user.getRoleCode());
        if (searchScope == NoteSearchScope.ALL && !roles.contains("admin")) {
    ...
    private Set<String> parseRoles(String roleCode) {
        if (oConvertUtils.isEmpty(roleCode)) {
            return new HashSet<>();
        }
    ...
    private Integer getTenantIdFromRequest(HttpServletRequest request) {
        try {
