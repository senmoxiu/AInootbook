...
@DisplayName("AinoteAiTaskServiceImpl 终态清理单元测试")
class AinoteAiTaskTerminalStateCleanupTest {
    ...
    @Test
    @DisplayName("completeTask: 完成任务时应清空 error_message 和 next_retry_at")
    void completeTask_shouldClearErrorMessageAndNextRetryAt_whenTransitioningToCompleted() {
        AinoteAiTask processingTask = new AinoteAiTask()
                .setId("task-complete")
                .setTaskStatus(1)
                .setTenantId(1)
                .setStartedAt(new Date(System.currentTimeMillis() - 3000L))
                .setErrorMessage("旧错误")
                .setNextRetryAt(new Date(System.currentTimeMillis() + 60000L));
        ...
        try (MockedStatic<SecurityUtils> securityUtils = mockTenantSubject(1)) {
            boolean completed = service.completeTask("task-complete", "{\"summary\":\"ok\"}");
            ...
            assertThat(wrapper.getParamNameValuePairs().values()).contains(2, "{\"summary\":\"ok\"}");
        }
    }

    @Test
    @DisplayName("failTask: 进入终态 FAILED 时应清空 next_retry_at")
    void failTask_shouldClearNextRetryAt_whenTaskEntersTerminalFailedState() {
        AinoteAiTask processingTask = new AinoteAiTask()
                .setId("task-terminal-fail")
                .setTaskType("summary")
                .setTaskStatus(1)
                .setRetryCount(2)
                .setTenantId(1)
        ...
        when(aiConfigService.getConfig(1))
                .thenReturn(AinoteAiConfig.defaults().setSummaryFailureMode("retry").setSummaryRetryLimit(2));
        when(aiTaskMapper.update(isNull(), any(UpdateWrapper.class))).thenReturn(1);

        try (MockedStatic<SecurityUtils> securityUtils = mockTenantSubject(1)) {
            boolean handled = service.failTask("task-terminal-fail", "仍然失败");
            ...
            verify(aiTaskMapper).update(isNull(), updateCaptor.capture());
            UpdateWrapper<AinoteAiTask> wrapper = updateCaptor.getValue();
            assertThat(wrapper.getSqlSet()).contains("task_status").contains("retry_count")
                    .contains("error_message").contains("next_retry_at");
            assertThat(wrapper.getParamNameValuePairs().values()).contains(2, 3);
            assertThat(stringParams(wrapper)).contains("仍然失败 (已达最大重试次数)");
        }
    }
