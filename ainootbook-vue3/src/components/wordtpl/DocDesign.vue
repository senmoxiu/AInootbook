        ...
        <a-layout-header class="space-between header">
          <div class="align-center header-title">
            ...
            <div v-else class="header-name">
              <a-input
                ref="nameRef"
                style="border-bottom: 1px solid #73a2e3; border-radius: 0"
                :bordered="false"
                v-model:value="formState.name"
                @pressEnter="saveTemplateNameClick"
                @blur="saveTemplateNameClick"
              ></a-input>
            </div>
          </div>
        </a-layout-header>
            ...
            <div class="menu" editor-component="menu">
              <div class="menu-item">
                ...
                <div class="jeecg-menu-item-upload" title="上传" @click="uploadWordFile">
                  <i />
                  <input ref="uploadWordFileRef" type="file" accept=".docx" @change="uploadDocxChange" style="display: none" />
                </div>
              ...
              <div class="menu-item">
                <div class="jeecg-menu-item-font" @click="fontFamilyClick('.jeecg-menu-item-font', '.options')">
                  ...
                  <div class="options">
                    <ul>
                      <li
                        v-for="item in fontStyleOption"
                        :data-family="item.value"
                        :style="{ 'font-family': item.value }"
                        @click="font(item.value, item.label)"
                        >{{ item.label }}</li
                      >
                    </ul>
                  </div>
                  ...
                  <div class="options">
                    <ul>
                      <li
                        ...
                        :title="`Ctrl+${isApple ? 'Option' : 'Alt'}+${index}`"
                        :data-level="item.value"
                        :style="{ fontSize: item.fontSize }"
                        @click="title(item.value)"
                        >{{ item.label }}</li
                      >
                    </ul>
                  </div>
              ...
              <div class="menu-item">
                ...
                <div class="jeecg-menu-item-image" @click="uploadImage">
                  <i title="图片" />
                  <input ref="uploadImageRef" id="image" type="file" accept=".png, .jpg, .jpeg, .svg, .gif" @change="uploadImageChange" />
                </div>
                ...
                <div class="jeecg-menu-item-separator" @click.stop="separatorClick">
                  ...
                  <div class="options page-mode-i">
                    <ul>
                      <li
                        class="page-mode-i"
                        v-for="item in separatorOption"
                        :data-separator="item.value"
                        @click.stop="separatorActiveClick(item.value)"
                      >
                        <i />
                      </li>
                    </ul>
                    <div class="more page-mode-i">
                      <div class="page-mode-i"
                        >颜色:<a-input @click.stop="" type="color" class="page-mode-i color" v-model:value="formState.separatorColor"
                      /></div>
                      <div class="page-mode-i"
                        >粗细(px):<a-input-number
                          @click.stop=""
                          type="tel"
                          class="page-mode-i lineWidth"
                          v-model:value="formState.separatorLineWidth"
                      /></div>
                ...
                <div class="options">
                  <ul>
                    <li
                      ...
                      :data-page-mode="item.value"
                      :class="item.value === 'paging' ? 'active' : ''"
                      @click="pageMode(item.value)"
                      >{{ item.label }}</li
                    >
                  </ul>
                </div>
            ...
            <div>
              ...
              <div class="paper-size" @click="fontFamilyClick('.paper-size', '.options')">
                ...
                <div class="options" title="纸张大小">
                  <ul>
                    <li
                      v-for="item in paperSizeOption"
                      :data-paper-size="item.value"
                      :class="item.label === 'A4' ? 'active' : ''"
                      @click="paperSize(item.value)"
                      >{{ item.label }}</li
                    >
                  </ul>
                </div>
              ...
              <div class="paper-direction" @click="fontFamilyClick('.paper-direction', '.options')">
                ...
                <div class="options">
                  <ul>
                    <li
                      v-for="item in paperDirectionOptions"
                      :data-paper-direction="item.value"
                      :class="item.value == 'vertical' ? 'active' : ''"
                      @click="paperDirection(item.value)"
                      >{{ item.label }}</li
                    >
                  </ul>
                </div>
