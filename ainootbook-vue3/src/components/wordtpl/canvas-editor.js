...
const EDITOR_ELEMENT_STYLE_ATTR = ['bold', 'color', 'highlight', 'font', 'size', 'italic', 'underline', 'strikeout', 'textDecoration'];
...
const TABLE_TD_ZIP_ATTR = ['conceptId', 'extension', 'externalId', 'verticalAlign', 'backgroundColor', 'borderTypes', 'slashTypes'];
const TABLE_CONTEXT_ATTR = ['tdId', 'trId', 'tableId'];
const TITLE_CONTEXT_ATTR = ['level', 'titleId', 'title'];
const LIST_CONTEXT_ATTR = ['listId', 'listType', 'listStyle'];
const CONTROL_CONTEXT_ATTR = ['control', 'controlId', 'controlComponent'];
const CONTROL_STYLE_ATTR = ['font', 'size', 'bold', 'highlight', 'italic', 'strikeout'];
const EDITOR_ELEMENT_CONTEXT_ATTR = [...TABLE_CONTEXT_ATTR, ...TITLE_CONTEXT_ATTR, ...LIST_CONTEXT_ATTR];
...
const IMAGE_ELEMENT_TYPE = [ElementType.IMAGE, ElementType.LATEX];
const BLOCK_ELEMENT_TYPE = [ElementType.BLOCK, ElementType.PAGE_BREAK, ElementType.SEPARATOR, ElementType.TABLE];
const INLINE_NODE_NAME = ['HR', 'TABLE', 'UL', 'OL'];
const VIRTUAL_ELEMENT_TYPE = [ElementType.TITLE, ElementType.LIST];
class ImageParticle {
  constructor(draw) {
    this.draw = draw;
...
function formatElementList(elementList, options) {
  ...
  if (
    ...
    (isHandleFirstElement &&
      (startElement == null ? void 0 : startElement.type) !== ElementType.LIST &&
      (((startElement == null ? void 0 : startElement.type) && startElement.type !== ElementType.TEXT) ||
        !START_LINE_BREAK_REG.test(startElement == null ? void 0 : startElement.value)))
  ) {
    elementList.unshift({
      value: ZERO,
    });
  ...
  while (i < elementList.length) {
    ...
    if (el.type === ElementType.TITLE) {
      ...
      elementList.splice(i, 1);
      const controlContext = pickObject(el, [...EDITOR_ELEMENT_CONTEXT_ATTR, ...EDITOR_ROW_ATTR]);
...
function formatElementContext(sourceElementList, formatElementList2, anchorIndex, options) {
  ...
  for (let e = 0; e < formatElementList2.length; e++) {
    ...
    if (isBreakWarped || (!copyElement.listId && targetElement.type === ElementType.LIST)) {
      ...
      (_b = targetElement.valueList) == null
        ? void 0
        : _b.forEach((valueItem) => {
...
`
        ...
        listElementListMap.forEach((listElementList, listIndex) => {
          ...
          text += `
${ulListStyleText || `${listIndex + 1}.`}${buildText(listElementList)}${
            isLast
              ? `
`
              : ``
...
function getSlimCloneElementList(elementList) {
  return deepCloneOmitKeys(elementList, ['metrics', 'style']);
}
...
function pasteByEvent(host, evt) {
  ...
  if (paste) {
    ...
    if ((overrideResult == null ? void 0 : overrideResult.preventDefault) !== false) return;
  }
...
function mousedown(evt, host) {
  ...
  if (~index2) {
    ...
    if (isDirectHitCheckbox && !isReadonly) {
      ...
      (((_a = curElement.control) == null ? void 0 : _a.type) === ControlType.CHECKBOX ||
        ((_b = curElement.control) == null ? void 0 : _b.type) === ControlType.RADIO)
    ) {
...
function mouseup(evt, host) {
  ...
  if (host.isAllowDrop) {
    ...
    if (
      ...
      ((_a = host.cachePositionContext) == null ? void 0 : _a.tdId) === positionContext.tdId
    ) {
    ...
    if (isContainControl) {
      ...
      const isAllowDragControl =
        ((!cacheStartElement2.controlId || cacheStartElement2.controlComponent === ControlComponent.PREFIX) &&
          (!cacheEndElement2.controlId || cacheEndElement2.controlComponent === ControlComponent.POSTFIX)) ||
        (cacheStartElement2.controlId === cacheEndElement2.controlId &&
          cacheStartElement2.controlComponent === ControlComponent.PREFIX &&
          cacheEndElement2.controlComponent === ControlComponent.POSTFIX) ||
        (((_b = cacheStartElement2.control) == null ? void 0 : _b.type) === ControlType.TEXT &&
          cacheStartElement2.controlComponent === ControlComponent.VALUE &&
          ((_c = cacheEndElement2.control) == null ? void 0 : _c.type) === ControlType.TEXT &&
          cacheEndElement2.controlComponent === ControlComponent.VALUE);
    ...
    const isOmitControlAttr =
      !isContainControl || !!elementList[range.startIndex].controlId || !control.getIsElementListContainFullControl(dragElementList);
...
function mousemove(evt, host) {
  ...
  if (host.isAllowDrag) {
    ...
    if (cacheStartIndex) {
      ...
      if (
        (dragElement == null ? void 0 : dragElement.type) === ElementType.IMAGE &&
        (dragElement.imgDisplay === ImageDisplay.SURROUND ||
          dragElement.imgDisplay === ImageDisplay.FLOAT_TOP ||
          dragElement.imgDisplay === ImageDisplay.FLOAT_BOTTOM)
      ) {
  ...
  if (isTable && startIsTable && (tdIndex !== startTdIndex || trIndex !== startTrIndex)) {
    ...
    if (
      (startElement == null ? void 0 : startElement.controlComponent) === ControlComponent.PLACEHOLDER &&
      (endElement == null ? void 0 : endElement.controlComponent) === ControlComponent.PLACEHOLDER &&
  ...
  if (isCrossRowCol) {
    ...
    if (positionContext.isDirectHit && positionContext.isImage) {
      ...
      if (!isCollapsed) {
        ...
        if (!elementList[index2 + 1]) return;
...
function enter(evt, host) {
  ...
  if (
    ...
    ((_a = elementList[endIndex + 1]) == null ? void 0 : _a.listId) !== endElement.listId
  ) {
    draw.getListParticle().unsetList();
    return;
  }
...
function left(evt, host) {
  ...
  const positionContext = position.getPositionContext();
  const { index: index2 } = cursorPosition;
  if (index2 <= 0 && !positionContext.isTable) return;
  const rangeManager = draw.getRange();
  const { startIndex, endIndex } = rangeManager.getRange();
  const isCollapsed = rangeManager.getIsCollapsed();
  const elementList = draw.getElementList();
  ...
  if (
    ...
    ((_a = elementList[index2]) == null ? void 0 : _a.controlComponent) === ControlComponent.PREFIX
  ) {
  ...
  if (isMod(evt)) {
    ...
    const moveStartIndex =
      evt.shiftKey && !isCollapsed && startIndex === (cursorPosition == null ? void 0 : cursorPosition.index) ? endIndex : startIndex;
...
function right(evt, host) {
  ...
  const positionList = position.getPositionList();
  const positionContext = position.getPositionContext();
  if (index2 > positionList.length - 1 && !positionContext.isTable) return;
  const rangeManager = draw.getRange();
  const { startIndex, endIndex } = rangeManager.getRange();
  const isCollapsed = rangeManager.getIsCollapsed();
  ...
  if (
    draw.getMode() === EditorMode.FORM &&
    control.getActiveControl() &&
    ((_a = elementList[index2 + 1]) == null ? void 0 : _a.controlComponent) === ControlComponent.POSTFIX
  ) {
    control.initNextControl({
...
function updown(evt, host) {
  ...
  if (
    ...
    ((isUp && cursorPosition.rowIndex === 0) || (!isUp && cursorPosition.rowIndex === draw.getRowCount() - 1))
  ) {
...
function input(data2, host) {
  ...
  const position = draw.getPosition();
  const cursorPosition = position.getCursorPosition();
  if (!data2 || !cursorPosition) return;
  const isComposing = host.isComposing;
  if (isComposing && ((_a = host.compositionInfo) == null ? void 0 : _a.value) === data2) return;
  const rangeManager = draw.getRange();
  if (!rangeManager.getIsCanInput()) return;
...
`,
  ...
  const inputData = splitText(text).map((value) => {
    ...
    if (
      isDesignMode ||
      (!((_a2 = copyElement.title) == null ? void 0 : _a2.disabled) && !((_b = copyElement.control) == null ? void 0 : _b.disabled))
    ) {
      const nextElement = elementList[endIndex + 1];
      if (
        !copyElement.type ||
        copyElement.type === TEXT ||
        (copyElement.type === HYPERLINK && (nextElement == null ? void 0 : nextElement.type) === HYPERLINK) ||
        (copyElement.type === DATE && (nextElement == null ? void 0 : nextElement.type) === DATE) ||
        (copyElement.type === SUBSCRIPT && (nextElement == null ? void 0 : nextElement.type) === SUBSCRIPT) ||
        (copyElement.type === SUPERSCRIPT && (nextElement == null ? void 0 : nextElement.type) === SUPERSCRIPT)
      ) {
        EDITOR_ELEMENT_COPY_ATTR.forEach((attr) => {
          if (attr === 'groupIds' && !(nextElement == null ? void 0 : nextElement.groupIds)) return;
...
function cut(host) {
  ...
  if (startIndex === endIndex) {
    ...
    for (let p = 0; p < positionList.length; p++) {
      ...
      if (position2.pageNo > curPageNo) break;
...
function copy(host) {
  ...
  if (!(copyElementList == null ? void 0 : copyElementList.length)) return;
...
function getWordRangeBySegmenter(host) {
  ...
  const paragraphText =
    ((_a = paragraphInfo == null ? void 0 : paragraphInfo.elementList) == null
      ? void 0
      : _a.map((e) => (!e.type || (e.type !== ElementType.CONTROL && TEXTLIKE_ELEMENT_TYPE.includes(e.type)) ? e.value : ZERO)).join('')) || '';
...
function threeClick(host) {
  ...
  while (upStartIndex > 0) {
    ...
    if (
      ...
      element.listId !== (preElement == null ? void 0 : preElement.listId) ||
      element.titleId !== (preElement == null ? void 0 : preElement.titleId)
    ) {
      break;
    }
    upCount++;
  ...
  while (downStartIndex < elementList.length) {
    ...
    if (
      (element.value === ZERO && !element.listWrap) ||
      element.listId !== (nextElement == null ? void 0 : nextElement.listId) ||
      element.titleId !== (nextElement == null ? void 0 : nextElement.titleId)
    ) {
      break;
    }
    downCount++;
  ...
  computePageRowPosition(payload) {
    ...
    for (let i = 0; i < rowList.length; i++) {
      ...
      for (let j = 0; j < curRow.elementList.length; j++) {
        ...
        const offsetY =
          (element.imgDisplay !== ImageDisplay.INLINE && element.type === ElementType.IMAGE) || element.type === ElementType.LATEX
...
class RangeManager {
  ...
  getIsCanInput() {
    ...
    return (
      (!startElement.controlId && !endElement.controlId) ||
      ((!startElement.controlId || startElement.controlComponent === ControlComponent.POSTFIX) &&
        (!endElement.controlId || endElement.controlComponent === ControlComponent.POSTFIX)) ||
      (!!startElement.controlId && endElement.controlId === startElement.controlId && endElement.controlComponent !== ControlComponent.POSTFIX)
    );
  }
  ...
  _drawImage(ctx, imageElement, width, height) {
    ...
    if (background.size === BackgroundSize.CONTAIN) {
      ...
      if (!background.repeat || background.repeat === BackgroundRepeat.NO_REPEAT) {
        ...
        let startY = 0;
        const repeatXCount =
          background.repeat === BackgroundRepeat.REPEAT || background.repeat === BackgroundRepeat.REPEAT_X
            ? Math.ceil((width * scale) / imageWidth)
            : 1;
        const repeatYCount =
          background.repeat === BackgroundRepeat.REPEAT || background.repeat === BackgroundRepeat.REPEAT_Y
  ...
  getMatchList(payload, originalElementList) {
    ...
    function searchClosure(payload2, type, elementList, restArgs) {
      ...
      const text = elementList
        .map((e) => (!e.type || (TEXTLIKE_ELEMENT_TYPE.includes(e.type) && e.controlComponent !== ControlComponent.CHECKBOX) ? e.value : ZERO))
        .filter(Boolean)
        .join('')
        .toLocaleLowerCase();
  ...
  render(ctx, pageIndex) {
    ...
    for (let s = 0; s < this.searchMatchList.length; s++) {
      ...
      if (searchMatch.type === EditorContext.TABLE) {
        ...
        position =
          (_b = (_a = elementList[tableIndex]) == null ? void 0 : _a.trList[trIndex].tdList[tdIndex]) == null ? void 0 : _b.positionList[index2];
      } else {
...
class ScrollObserver {
  ...
  getElementVisibleInfo(element) {
    ...
    const viewHeight =
      this.scrollContainer === document ? Math.max(document.documentElement.clientHeight, window.innerHeight) : this.scrollContainer.clientHeight;
...
class TableParticle {
  ...
  getTableHeight(element) {
    const trList = element.trList;
    if (!(trList == null ? void 0 : trList.length)) return 0;
...
class Watermark {
  ...
  render(ctx) {
    const {
      watermark: { data: data2, opacity, font, size, color, repeat, gap },
      scale,
    } = this.options;
    const width = this.draw.getWidth();
    const height = this.draw.getHeight();
...
class ControlSearch {
    ...
    const computeHighlight = (elementList, restArgs) => {
      ...
      while (i < elementList.length) {
        ...
        const highlightIndex = this.highlightList.findIndex(
          (highlight2) => highlight2.id === element.controlId || (currentControl.conceptId && currentControl.conceptId === highlight2.conceptId)
        );
        ...
        const controlElementList = elementList
          ...
          .map((element2) => (element2.controlComponent === ControlComponent.VALUE ? element2 : { value: ZERO }));
  ...
  renderHighlightList(ctx, pageIndex) {
    var _a, _b, _c;
    if (!((_a = this.highlightMatchResult) == null ? void 0 : _a.length)) return;
    ...
    for (let s = 0; s < this.highlightMatchResult.length; s++) {
      ...
      if (searchMatch.tableId) {
        const { tableIndex, trIndex, tdIndex, index: index2 } = searchMatch;
        position =
          (_c = (_b = elementList[tableIndex]) == null ? void 0 : _b.trList[trIndex].tdList[tdIndex]) == null ? void 0 : _c.positionList[index2];
      } else {
        position = positionList[searchMatch.index];
      }
      if (!position) continue;
  ...
  keydown(evt) {
    ...
    if (evt.key === KeyMap.Backspace) {
      ...
      if (startIndex !== endIndex) {
        ...
        if (
          (startElement.controlComponent === ControlComponent.PREFIX && endNextElement.controlComponent === ControlComponent.PLACEHOLDER) ||
...
class TextControl {
  ...
  setValue(data2, context = {}, options = {}) {
    ...
    const startElement = elementList[startIndex];
    const anchorElement =
      (startElement.type && !TEXTLIKE_ELEMENT_TYPE.includes(startElement.type)) || startElement.controlComponent === ControlComponent.PREFIX
...
class Control {
  ...
  setValueById(payload) {
    ...
    this.destroyControl();
    const data2 = [this.draw.getHeaderElementList(), this.draw.getOriginalMainElementList(), this.draw.getFooterElementList()];
    ...
    function getControlElementList(elementList) {
      for (let e = 0; e < elementList.length; e++) {
        ...
        if (element.controlId) {
          const controlElement = omitObject(element, [...TITLE_CONTEXT_ATTR, ...LIST_CONTEXT_ATTR]);
          controlElementList.push(controlElement);
        }
      }
    }
    const data2 = [this.draw.getHeader().getElementList(), this.draw.getOriginalMainElementList(), this.draw.getFooter().getElementList()];
    for (const elementList of data2) {
  ...
  setMinWidthControlInfo(option) {
    ...
    if (!((_a = rowElement.control) == null ? void 0 : _a.minWidth)) return;
    const { scale } = this.options;
    const controlMinWidth = rowElement.control.minWidth * scale;
    let controlFirstElement = null;
    if (
      ((_b = rowElement.control) == null ? void 0 : _b.minWidth) &&
      (((_c = rowElement.control) == null ? void 0 : _c.rowFlex) === RowFlex.CENTER ||
        ((_d = rowElement.control) == null ? void 0 : _d.rowFlex) === RowFlex.RIGHT)
    ) {
...
const encodedJs$2 =
  '';
    ...
    this.pageContainer.addEventListener(
      ...
      throttle((evt) => {
        ...
        if (evt.target instanceof HTMLCanvasElement) {
          ...
          this._updateZoneTip(
            this.zone.getZone() === EditorZone.MAIN && (mousemoveZone === EditorZone.HEADER || mousemoveZone === EditorZone.FOOTER),
            evt.x,
            evt.y
          );
        } else {
...
class ListParticle {
  ...
  drawListStyle(ctx, row, position) {
    ...
    for (let i = 1; i < elementList.length; i++) {
      const element = elementList[i];
      if ((element == null ? void 0 : element.type) !== ElementType.TAB) break;
...
class Placeholder {
  ...
  render(ctx) {
    const {
      placeholder: { data: data2, font, size, color, opacity },
    } = this.options;
    if (!data2) return;
    this._recovery();
    this.elementList = [
...
class Group {
  ...
  getElementListByGroupId(elementList, groupId) {
    ...
    for (let e = 0; e < elementList.length; e++) {
      ...
      if ((_a = element == null ? void 0 : element.groupIds) == null ? void 0 : _a.includes(groupId)) {
        ...
        const nextElement = elementList[e + 1];
        if (!((_b = nextElement == null ? void 0 : nextElement.groupIds) == null ? void 0 : _b.includes(groupId))) break;
      }
    }
    return groupElementList;
  }
  ...
  getContextByGroupId(elementList, groupId) {
    ...
    for (let e = 0; e < elementList.length; e++) {
      ...
      const nextElement = elementList[e + 1];
      if (
        ((_a = element.groupIds) == null ? void 0 : _a.includes(groupId)) &&
        !((_b = nextElement == null ? void 0 : nextElement.groupIds) == null ? void 0 : _b.includes(groupId))
      ) {
        return {
          isTable: false,
          startIndex: e,
...
class LineNumber {
  ...
  render(ctx, pageNo) {
    const {
      scale,
      lineNumber: { color, size, font, right: right2, type },
    } = this.options;
...
class Draw {
  ...
  isDisabled() {
    ...
    const { startIndex, endIndex } = this.range.getRange();
    const elementList = this.getElementList();
    if ((_a = this.getTd()) == null ? void 0 : _a.disabled) return true;
    if (startIndex === endIndex) {
      const startElement = elementList[startIndex];
      const nextElement = elementList[startIndex + 1];
      return !!(
        (((_b = startElement == null ? void 0 : startElement.title) == null ? void 0 : _b.disabled) &&
          ((_c = nextElement == null ? void 0 : nextElement.title) == null ? void 0 : _c.disabled)) ||
        (((_d = startElement == null ? void 0 : startElement.control) == null ? void 0 : _d.disabled) &&
          ((_e = nextElement == null ? void 0 : nextElement.control) == null ? void 0 : _e.disabled))
      );
    }
  ...
  insertElementList(payload) {
    ...
    if (activeControl && this.control.getIsRangeWithinControl()) {
      ...
      if (
        ...
        preElement &&
        !preElement.listId &&
        (preElement == null ? void 0 : preElement.value) === ZERO &&
      ...
      if (!this.control.getActiveControl()) {
        ...
        while (deleteIndex >= start) {
          ...
          if (
            isDesignMode ||
            (tdDeletable !== false &&
              ((_c = deleteElement == null ? void 0 : deleteElement.control) == null ? void 0 : _c.deletable) !== false &&
              ((_d = deleteElement == null ? void 0 : deleteElement.title) == null ? void 0 : _d.deletable) !== false)
          ) {
  ...
  computeRowList(payload) {
    ...
    if (elementList.length) {
      rowList.push({
        ...
        rowIndex: 0,
        rowFlex:
          ((_a = elementList == null ? void 0 : elementList[0]) == null ? void 0 : _a.rowFlex) ||
          ((_b = elementList == null ? void 0 : elementList[1]) == null ? void 0 : _b.rowFlex),
      });
    }
    let x = startX;
    let y = startY;
    ...
    for (let i = 0; i < elementList.length; i++) {
      ...
      if (element.type === ElementType.IMAGE || element.type === ElementType.LATEX) {
        ...
        if (isPagingMode) {
          ...
          if (
            curPagePreHeight + element.trList[0].height + rowMarginHeight > height2 ||
      ...
      if (this.options.wordBreak === WordBreak.BREAK_WORD) {
        if (
          (!(preElement == null ? void 0 : preElement.type) || (preElement == null ? void 0 : preElement.type) === ElementType.TEXT) &&
      ...
      const isForceBreak =
        ...
        (preElement == null ? void 0 : preElement.type) === ElementType.TABLE ||
        (preElement == null ? void 0 : preElement.type) === ElementType.BLOCK ||
        element.type === ElementType.BLOCK ||
        (preElement == null ? void 0 : preElement.imgDisplay) === ImageDisplay.INLINE ||
        element.imgDisplay === ImageDisplay.INLINE ||
        (preElement == null ? void 0 : preElement.listId) !== element.listId ||
      ...
      if (isWrap) {
        ...
        if (
          ...
          ((_j = rowElement.control) == null ? void 0 : _j.indentation) === ControlIndentation.VALUE_START
        ) {
          const preStartIndex = curRow.elementList.findIndex(
            (el) => el.controlId === rowElement.controlId && el.controlComponent !== ControlComponent.PREFIX
          );
          if (~preStartIndex) {
      ...
      if (isWrap || i === elementList.length - 1) {
        ...
        if (
          !curRow.isSurround &&
          ((preElement == null ? void 0 : preElement.rowFlex) === RowFlex.JUSTIFY ||
            ((preElement == null ? void 0 : preElement.rowFlex) === RowFlex.ALIGNMENT && isWidthNotEnough))
        ) {
          const rowElementList =
            ((_k = curRow.elementList[0]) == null ? void 0 : _k.value) === ZERO ? curRow.elementList.slice(1) : curRow.elementList;
      ...
      if (isWrap) {
        ...
        if (
          ...
          !isFromTable &&
          pageHeight &&
          (y - startY + mainOuterHeight + height > pageHeight || element.type === ElementType.PAGE_BREAK)
        ) {
          y = startY;
      ...
      for (let j = 0; j < curRow.elementList.length; j++) {
        ...
        if (element.underline || ((_d = element.control) == null ? void 0 : _d.underline)) {
          ...
          this.underline.recordFillInfo(
            ctx,
            x - offsetX,
            y + curRow.height - rowMargin + offsetY2,
            metrics.width + offsetX,
            0,
            color,
            (_f = element.textDecoration) == null ? void 0 : _f.style
          );
        } else if (
          (preElement == null ? void 0 : preElement.underline) ||
          ((_g = preElement == null ? void 0 : preElement.control) == null ? void 0 : _g.underline)
        ) {
          this.underline.render(ctx);
        }
        if (element.strikeout) {
          if (!element.type || TEXTLIKE_ELEMENT_TYPE.includes(element.type)) {
            if (
              ...
              ((preElement.type === ElementType.SUBSCRIPT && element.type !== ElementType.SUBSCRIPT) ||
                (preElement.type === ElementType.SUPERSCRIPT && element.type !== ElementType.SUPERSCRIPT) ||
  ...
  _drawFloat(ctx, payload) {
    ...
    for (let e = 0; e < floatPositionList.length; e++) {
      ...
      if (
        (pageNo === floatPosition.pageNo || floatPosition.zone === EditorZone.HEADER || floatPosition.zone == EditorZone.FOOTER) &&
        element.imgDisplay &&
...
class CommandAdapt {
    ...
    if (selection == null ? void 0 : selection.length) {
      const isSetUnderline = selection.some(
        (s) =>
          ...
          (textDecoration && !s.textDecoration) ||
          (textDecoration && s.textDecoration && !isObjectEqual(s.textDecoration, textDecoration))
      );
      selection.forEach((el) => {
  ...
  updateElementById(payload) {
    ...
    const getElementListFnList = [this.draw.getOriginalMainElementList, this.draw.getHeaderElementList, this.draw.getFooterElementList];
  ...
  locationControl(controlId, options) {
    ...
    function location(elementList, zone2) {
      ...
      while (i < elementList.length) {
        ...
        if ((element == null ? void 0 : element.controlId) !== controlId) continue;
        let curIndex = i - 1;
        if (isLocationAfter) {
          curIndex -= 1;
  ...
  insertControl(payload) {
    ...
    if (!copyElement) return;
    const cloneAttr = [...TABLE_CONTEXT_ATTR, ...EDITOR_ROW_ATTR, ...LIST_CONTEXT_ATTR];
  ...
  getTitleValue(payload) {
    ...
    const getValue = (elementList, zone2) => {
      ...
      while (i < elementList.length) {
        ...
        if (((_a = element == null ? void 0 : element.title) == null ? void 0 : _a.conceptId) !== conceptId) continue;
        const valueList = [];
        let j = i;
        while (j < elementList.length) {
          const nextElement = elementList[j];
  ...
  getPositionContextByEvent(evt) {
    ...
    if (!isDirectHit || (zone2 && zone2 !== this.zone.getZone())) return null;
    let element = null;
    const elementList = this.draw.getOriginalElementList();
    let position = null;
    const positionList = this.position.getOriginalPositionList();
  ...
  insertTitle(payload) {
    ...
    if (!copyElement) return;
    const cloneAttr = [...TABLE_CONTEXT_ATTR, ...EDITOR_ROW_ATTR, ...LIST_CONTEXT_ATTR];
    (_a = cloneElement.valueList) == null
      ? void 0
      : _a.forEach((valueItem) => {
...
const controlMenus = [
  {
    ...
    when: (payload) => {
      ...
      return (
        ...
        !!((_a = payload.startElement) == null ? void 0 : _a.controlId) &&
...
const {
  IMAGE: { CHANGE, SAVE_AS, TEXT_WRAP, TEXT_WRAP_EMBED, TEXT_WRAP_UP_DOWN, TEXT_WRAP_SURROUND, TEXT_WRAP_FLOAT_TOP, TEXT_WRAP_FLOAT_BOTTOM },
} = INTERNAL_CONTEXT_MENU_KEY;
const imageMenus = [
  {
    key: CHANGE,
...
class ContextMenu {
  constructor(draw, command) {
    ...
    this._handleSideEffect = (evt) => {
      if (this.contextMenuContainerList.length) {
        ...
        const contextMenuDom = findParent(
          target,
          (node) => !!node && node.nodeType === 1 && node.getAttribute(EDITOR_COMPONENT) === EditorComponent.CONTEXTMENU,
          true
        );
        if (!contextMenuDom) {
          this.dispose();
    ...
    this.container = draw.getContainer();
    this.context = null;
    this.contextMenuList = [...globalMenus, ...tableMenus, ...imageMenus, ...controlMenus, ...hyperlinkMenus];
  ...
  _setHoverStatus(payload, status) {
    var _a;
    if (status) {
      (_a = payload.parentNode) == null
        ? void 0
        : _a.querySelectorAll(`${EDITOR_PREFIX}-contextmenu-item`).forEach((child) => child.classList.remove('hover'));
...
class Shortcut {
  ...
  _execute(evt, shortCutList) {
    ...
    for (let s = 0; s < shortCutList.length; s++) {
      ...
      if (
        (shortCut.mod ? isMod(evt) === !!shortCut.mod : evt.ctrlKey === !!shortCut.ctrl && evt.metaKey === !!shortCut.meta) &&
