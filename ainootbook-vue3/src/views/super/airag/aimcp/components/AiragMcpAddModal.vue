      ...
      <div class="mcp-content">
        <BasicForm @register="registerForm">
          ...
          <template #endpoint="{ model, field }">
            <a-input v-model:value="model[field]" :placeholder="endpointPlaceholder" autocomplete="new-password" />
          </template>
          ...
          <template #headers>
            ...
            <a-form-item-rest>
              <div class="headers-table-wrapper">
                <a-button type="primary" size="small" @click="addHeaderRow" style="margin-bottom: 8px"> 添加 </a-button>
                <div class="headers-table-container">
                  <a-table :dataSource="headersData" :columns="headersColumns" :pagination="false" size="small" bordered>
...
<script lang="ts" setup>
  ...
  const endpointPlaceholder = computed(() => {
    ...
    return typeValue.value === 'sse' || typeValue.value === 'http'
      ? '请输入服务端点URL，例如：https://example.com/stream'
      : '请输入启动命令，例如：./start-mcp-service';
  });
