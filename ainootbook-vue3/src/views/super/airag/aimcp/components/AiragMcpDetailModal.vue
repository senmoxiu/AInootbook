<template>
  <BasicModal destroyOnClose @register="registerModal" :canFullscreen="false" width="720px" wrapClassName="airag-mcp-detail-modal">
    <template #title>
      <div class="detail-modal-title">
        <span>详情</span>
        <a-button class="detail-edit-btn" type="text" @click="emitEdit" :title="'编辑'">
      ...
      <div class="tools-wrapper">
        ...
        <div class="tools-grid" v-if="(pluginTools && pluginTools.length) || (tools && tools.length)">
          ...
          <template v-if="record?.category === 'plugin'">
            <div v-for="tool in pluginTools" :key="tool.name" class="tool-item" @click="handleEditTool(tool)">
              <div class="tool-header-item" @click.stop>
                <div class="tool-name" :title="tool.name">{{ tool.name }}</div>
                <div class="tool-actions">
                  <a-switch v-model:checked="tool.enabled" size="small" @change="handleToolEnabledChange(tool)" @click.stop />
                  <a-button type="text" size="small" @click.stop="handleEditTool(tool)" :title="'编辑工具'">
                    <Icon icon="ant-design:edit-outlined" :size="16" />
                  </a-button>
                  <a-button type="text" danger size="small" @click.stop="handleDeleteTool(tool)" :title="'删除工具'">
