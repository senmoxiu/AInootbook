    ...
    <div class="content">
      <div class="header-title" v-if="type === 'view' && headerTitle">
        <div class="title-content">
          ...
          <a-button v-if="hasExtraFlowInputs" type="text" class="edit-btn" @click="handleEditSettings" title="参数设置">
              ...
              <div class="textarea-action-bar">
                <div class="left-actions">
                  ...
                  <a-tooltip v-if="showThink" :title="enableThink ? '关闭深度思考' : '开启深度思考'">
                    <a-button class="sendBtn webSearchBtn" type="text" :class="{ enabled: enableThink }" @click="toggleThink">
                      <svg
                        style="margin-right: 6px"
                        :style="enableThink ? { color: '#06f' } : { color: '#3d4353' }"
                        width="16"
                        height="16"
                        viewBox="0 0 14 14"
                        fill="none"
                  ...
                  <a-tooltip v-if="showWebSearch" :title="enableSearch ? '关闭联网搜索' : '开启联网搜索'">
                    <a-button class="sendBtn webSearchBtn" type="text" :class="{ enabled: enableSearch }" @click="toggleWebSearch">
                  ...
                  <a-tooltip v-if="showDraw" :title="enableDraw ? '关闭图像生成' : '开启图像生成'">
                    <a-button class="sendBtn webSearchBtn" type="text" :class="{ enabled: enableDraw }" @click="handleGenerateImage">
                      <svg
                        style="margin-right: 6px"
                        :style="enableDraw ? { color: '#06f' } : { color: '#3d4353' }"
                        width="16"
                        height="16"
                        viewBox="0 0 24 24"
                        fill="currentColor"
...
<script setup lang="ts">
  ...
  async function aiReConnection() {
    ...
    if (chat) {
      ...
      const result = await defHttp
        .get(
          { url: '/airag/chat/receive/' + requestId, adapter: 'fetch', responseType: 'stream', timeout: 5 * 60 * 1000 },
          { isTransformResponse: false }
        )
        .catch(async (err) => {
          loading.value = false;
  ...
  function handleChangeFile(info) {
    ...
    fileUrlList.value = fileList.value.filter((item) => item.status === 'done' && item.response).map((item) => item.response.message);
  }
